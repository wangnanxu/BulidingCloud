        ...
        public List<Account.Dtos.UserDto> SearchUserByName(string nameKeyword, int pageNumber, int pageSize, out int amount)
        {
            try
            {
                using (var db = new BCBackContext())
                {
                    ...
                    if (pageNumber < 1)
                        pageNumber = 1;
                    return list.OrderBy(x => x.UserID).Skip(pageSize * (pageNumber - 1)).Take(pageSize).Select(obj => new Account.Dtos.UserDto
                    {
                        UserID = obj.UserID,
                        Name = obj.Name,
                        Mobile = obj.Mobile,
        ...
        public List<Account.Dtos.UserLoginStateDto> GetUserLoginStateList(string UserName, int pageNumber, int pageSize, out int amount)
        {
            try
            {
                ...
                using (var db = new BCEnterpriseContext())
                {
                    ...
                    if (pageNumber < 1)
                        pageNumber = 1;
                    return list.OrderByDescending(x => x.LoginTime).Skip(pageSize * (pageNumber - 1)).Take(pageSize).ToList();
                }
            }
            catch (Exception e)
            {
