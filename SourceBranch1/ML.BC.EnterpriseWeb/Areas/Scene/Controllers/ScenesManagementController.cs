    ...
    public class ScenesManagementController : BCControllerBase
    {
        ...
        public ActionResult CheckSceneItem(string SceneItemID, int status, string content)
        {
            ...
            result.Try(() =>
            {
                ...
                {
                    UserID = _userID,
                    Content = (status == 1 ? "[审核通过]" : "[需要整改]") + content ?? "",
                };
                if (status == 1)
                {
                    result.Value = _sceneItemService.AddCommentItem(SceneItemID, comment, ItemStatus.Pass) != null;
                }
                else
                {
                    result.Value = _sceneItemService.AddCommentItem(SceneItemID, comment, ItemStatus.Redo) != null;
                }
        ...
        public ActionResult AddSceneItemComment(string SceneItemID, string comment)
        {
            string _enterpriseID = BCSession.User.EnterpriseID;
            int? _departmentID = BCSession.User.DepartmentID;
            string _userID = BCSession.User.UserID;
            ...
            result.Try(() =>
            {
                ...
                try
                {
                    result.Value = !(_sceneItemService.AddCommentItem(SceneItemID, cmt, ItemStatus.Normal) == null);
                }
                catch (Exception e)
                {
        ...
        public ActionResult ArchiveSceneItem(string SceneItemID)
        {
            ...
            result.Try(() =>
            {
                ...
                result.Value = _sceneItemService.SetSceneItemStatus(SceneItemID, ItemStatus.Final, DateTime.Now);
            });
            return result;
        }
