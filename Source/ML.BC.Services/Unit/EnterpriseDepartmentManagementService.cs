    ...
    public class EnterpriseDepartmentManagementService : IEnterpriseDepartmentManagementService
    {
        ...
        public bool Delete(int departmentId, string enterpriseId)
        {
            try
            {
                using (var db = new BCEnterpriseContext())
                {
                    var mydepartments = GetOwnerDepartment(departmentId, db).Select(x => x.DepartmentID.ToString()).ToList();
                    mydepartments.Add(departmentId.ToString());
                    var projectlist = db.Projects.Where(p => p.EnterpriseID.Equals(enterpriseId) && p.Deleted == false).ToList();
                    var project =
                        projectlist.FirstOrDefault(
                            p => p.Departments.Split("|".ToCharArray()).Any(x => mydepartments.Contains(x)));
                    if (null != project)
                    {
                        throw new KnownException("该部门或子部门下存在项目，请先删除项目");
                    }
                    var users = db.FrontUsers.FirstOrDefault(x => mydepartments.Contains((x.DepartmentID ?? -5).ToString()) && x.EnterpiseID.Equals(enterpriseId));
                    if (null == users)
