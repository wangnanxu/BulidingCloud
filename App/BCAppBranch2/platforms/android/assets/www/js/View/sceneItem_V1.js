...
function ShowData(jsondatas, islocal) {
	...
	for (var i = _length - 1; i >= 0; i--) {
		...
		if (_tem != "") {
			...
			_tem = _tem.replace(/{OPTION}/g, ShowOption(messageid, jsondata.UserID, jsondata.Type)); //组装按钮
			...
			if (jsondata.Type == "1") {
				...
				_tem = _tem.replace(/{Sign}/g, "过程");
			} else if (jsondata.Type == "4") {
				_tem = _tem.replace(/{Sign}/g, "安全");
			} else if (jsondata.Type == "8") {
				_tem = _tem.replace(/{Sign}/g, "临检");
			} else if (jsondata.Type == "16") {
				_tem = _tem.replace(/{Sign}/g, "交底");
			} else if (jsondata.Type == "32") {
				_tem = _tem.replace(/{Sign}/g, "签退");
			}else if (jsondata.Type == "64") {
				_tem = _tem.replace(/{Sign}/g, "完工");
			}else{
				_tem = _tem.replace(/{Sign}/g, "");{Status}
			}
...
function ShowOption(messageid, userid, type) {
...
function ExamineSceneMessage(messageid, type) {
	...
	function ExamineConfirm(results) {
		...
		var _jsondata = {
			...
			Content: results.buttonIndex == 1 ? "审核通过，" + results.input1 : "需要整改，" + results.input1 + "&nbsp;" + _optem,
			Time: _time
		}
		var SOP = new ShowOp();
		if (results.buttonIndex == 1) {
			OPLC.AddComment(messageid, _guid, _time, "审核通过，" + results.input1, 1);
			SOP.Comment(_jsondata);
			OPSI.Comment(messageid, _guid, "审核通过，" + results.input1, 1, _time) //通过
		} else if (results.buttonIndex == 2) {
			OPLC.AddComment(messageid, _guid, _time, "需要整改，" + results.input1, 2); //本地数据库操作
			SOP.Comment(_jsondata); //界面操作
			OPSI.Comment(messageid, _guid, "需要整改，" + results.input1, 2, _time) //服务器请求
