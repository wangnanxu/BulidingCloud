...
function SceneItemSign(type) {
	SignConfirm(1);
	//	if (1) {
	//		NotificationConfirm("确定要签到吗?", "签到提示!", "签到,取消", SignConfirm);
	//	} else {
	//		NotificationConfirm("确定要签退吗?", "签到提示!", "签退,取消", SignConfirm);
	//	}
...
function OfflineScentItem() {
	...
	SelectDBUpload(_sceneid, [2, 3, 4], function(jsondatas) {
		...
		var _jsonstate2 = [];
		for (var i = 0; i < _length; i++) {
			if (parseInt(_jsondata[i].State) == 2) {
				_jsonstate2.push(_jsondata[i]);
			} else if (parseInt(_jsondata[i].State) == 3) {
				PostOfflineCommentsData(_jsondata[i].MessageID, _jsondata[i].Comments, _jsondata[i].Status, function() {
					ChangeState(_jsondata[i], 1);
				})
			} else if (parseInt(_jsondata[i].State) == 4) {
		...
		if (_jsonstate2.length > 0) {
			UploadSceneItemLX(_jsonstate2);
		}
	});
};
/*上传数据*/
...
/*离线上传数据*/
function UploadSceneItemLX(jsondatas) {
	GetPosition(function(_position) { //获取经纬度
		if (_position != null) {
			var _statelength = jsondatas.length;
			for (var a = 0; a < _statelength; a++) {
				UploadOnData(jsondatas[a]);
			}

			function UploadOnData(jsondata) {
				var _length = 0;
				if (jsondata.Images != "" && jsondata.Images != "[]") {
					var _images = JSON.parse(jsondata.Images);
					_length = _images.length;
...
function DownLocalPicture(orgimgurl, id, messageid, localthumig) {
	if (orgimgurl) {
		...
		DownLoad_New(orgimgurl, 0, "", callback); //下载图片
...
function DeletePhoto(messageid, photourl) {
	dbBase.OpenTransaction(function(tx) {
		dbBase.SelectTable(tx, "select Images from tb_SceneMessageComments where MessageID=?", [messageid], function(adata) {
			if (adata) {
				var _json = JSON.parse(adata.item(0).Images);
				var _len = _json.length;
				var i = _len - 1;
				for (i; i >= 0; i--) {
					if (_json[i].OriginalPicture.indexOf(photourl) != -1) {
						_json.splice(i, 1);
						break;
					}
				}
				var _str = JSON.stringify(_json);
				dbBase.SaveOrUpdateTable(tx, "tb_SceneMessageComments", ["Images"], [_str], "MessageID", messageid);
			}
		})
	})
...
function openPhotoSwipe(_index, _messageid,status,isexamine) {
	var pswpElement = document.querySelectorAll('.pswp')[0];
	if (!PhotoSwipePictures[_messageid]) {
		PhotoSwipePictures[_messageid] = NewGetImages(_messageid);
	}
	var nowitems = NewGetImages(_messageid);
	if (PhotoSwipePictures[_messageid].length != nowitems) {
		PhotoSwipePictures[_messageid] = nowitems;
	}
	var items = PhotoSwipePictures[_messageid];
	...
	gallery.listen('shareLinkClick', function(e, target) {
		if(status == "3"||(status == "2" && (isexamine == "true" || isexamine))){
			alert("照片已归档，无法删除")
		}else{
			deletePicture(_messageid);
			PhotoSwipePictures[_messageid] = gallery.items;
		}
	});
	gallery.listen('afterChange', function() {
		downloadPicture(_messageid)
	});
};
...
function deletePicture(messageid) {
	...
	var _url = gallery.items[_index].src;
	...
	var _messageid = messageid.replace("Gallery_", "");
	var _names = _url.indexOf('=') != -1 ? _url.split('=') : _url.split('/');
	var _name = _names[_names.length - 1];
	DeletePhoto(_messageid, _name);
	PostDeletePicture(_messageid, _name);
};

function downloadPicture(messageid) {
	var index = gallery.getCurrentIndex();
	var _id = gallery.items[index].id;
	if (!gallery.items[index].isOrg) {
		var _orgurl = $("#" + _id).attr("alt");
		LoadPictureNew(_orgurl, null, function(localurl) {
			var img = new Image();
			if (CheckPlatform()) {
				img.src = _orgurl;
			} else {
			...
			setTimeout(function() {
				...
				PhotoSwipePictures[messageid] = gallery.items;
			}, 500)
		})
	}
}