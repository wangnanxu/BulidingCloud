...
var PHOTOSWIPE_ADDSCENEIMG_TEM = "<img id='thu_{ImageName}' src='../img/loadimg.gif' alt='{ImageOrgURI}' onclick=openPhotoSwipe('{index}','Gallery_{MessageID}','{Status}','{IsExamine}') onload=\"LoadPictureNew('{ImageURI}','thu_{ImageName}');return false;\" onerror='this.src=\"../img/loadimg.gif\"' style='width:100px;height:100px' />";
/*显示除评论和头像外的数据
 * jsondata:json数据
 * temid：模板id
 */
...
function ShowOption(messageid, userid, type, status, isexamine) {
	if (CheckPermission(false)) {
		var _optemarray = [];
		var _optem = "";
		if (userInfo.VerifySceneData&&!(status == "2" && (isexamine == "true" || isexamine))) {
			_optem = "<a href='#' onclick=ExamineSceneMessage('{MessageID}','" + type + "','"+userid+"')>[审核]</a>";
			_optemarray.push(_optem.replace(/{MessageID}/g, messageid));
		}
		_optem = "<a href='#' onclick=CommentSceneMessage('{MessageID}','{Status}')>[评论]</a>";
		_optem = _optem.replace(/{MessageID}/g, messageid);
		_optem = _optem.replace(/{Status}/g, status);
		_optemarray.push(_optem);
		if (userInfo.UserID == userid&&!(status == "2" && (isexamine == "true" || isexamine))) {
			_optem = "<a href='#' onclick=DeleteSceneMessage('{MessageID}')>[删除]</a>";
			_optemarray.push(_optem.replace(/{MessageID}/g, messageid));
		}
		if (userInfo.ArchiveSceneData&&!(status == "2" && (isexamine == "true" || isexamine))) {
...
function ShowImages(jsondata, local) {
	...
	if (jsondata.Images != "" && jsondata.Images != null) {
		...
		for (var i = 0; i < _length; i++) {
			...
			_imgtem = _imgtem.replace(/{Status}/g, jsondata.Status);
			_imgtem = _imgtem.replace(/{IsExamine}/g, jsondata.IsExamine);
...
//审核消息
function ExamineSceneMessage(messageid, type,userid) {
...
//评论消息
function CommentSceneMessage(messageid, status) {
	navigator.notification.prompt(
		'输入评论内容!',
		CommentConfirm,
		'评论', ['确认', '取消']
	...
	function CommentConfirm(results) {
		if (results.buttonIndex == 1) {
			if (results.input1 != "") {
				...
				var SOP = new ShowOp();
				SOP.Comment(_jsondata);
				OPSI.Comment(messageid, _guid, results.input1, status, _time) //整改
			} else {
				alert("评论内容不能为空");
			}
		}
