...
//请求服务器清除同步状态后本地清除缓存
function PostClearCache(callback) {
	if (onLine) {
		var _data = {
			Token: userInfo.Token
		}
		$.ajax({
			type: "POST",
			url: baseurl + '/APIS/SyncData/ClearCache',
			data: _data,
			dataType: "json",
...
function PostGetAppVersion(callback){
	if (onLine) {
		$.post(baseurl + '/APIS/Account/GetAppVersion',"", function(adata) {
			if(adata.Success){
				callback(adata.Value);
			}else{
				alert("获取最新版本信息失败");
