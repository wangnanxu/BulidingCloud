				...
				for (var i = 0; i < _len; i++) {
					switch (data[i].EntityName) {
						...
						case "SceneType": //删除现场类型
							_delete.DeleteSceneType(data[i].EntityID);
							break;
						default:
							break;
					}
		...
		this.SyncSceneAndProject = function(jsondata) {
			if (jsondata) {
				...
				dbBase.OpenTransaction(function(tx) {
					...
					for (var k = 0; k < _lentype; k++) {
						//存储现场类型数据
						dbBase.SaveOrUpdateTable(tx, "tb_SceneTypes", ['SceneTypeID', 'SceneTypeName', 'ParentID', 'Available', 'EnterpriseID'], [_scenetype[k].SceneTypeID, _scenetype[k].SceneTypeName, _scenetype[k].ParentID, _scenetype[k].Available, _scenetype[k].EnterpriseID], "SceneTypeID", [_scenetype[k].SceneTypeID]);
					}
				})
			}
		},
...
var HandleDeleteData = function() {
	...
	this.DeleteMessage = function(jsondata) {
		...
		this.DeleteSceneType = function(typeID) {
			if (typeID) {
				dbBase.OpenTransaction(function(tx) {
...
function PushNewToSceneItem(tx, jsondata, callback) {
	if (jsondata) {
		...
		dbBase.SaveOrUpdateTable(tx, "tb_SceneMessageComments", ["MessageID", "SceneID", "UserID", "UserPicture", "UserPictureURI", "UserName", "Address", "CreateTime", "Description", "Images", "Comments", "Status", "Type", "State", "PictureGuid","Relation","IsExamine"], [jsondata.Id, jsondata.SceneID, jsondata.UserID, jsondata.UserPicture, "", jsondata.UserName, jsondata.Address, jsondata.CreateTime, jsondata.Description, _strimages, _strcomments, jsondata.Status, jsondata.Type, 1, jsondata.PictureGuid,jsondata.Relation,jsondata.IsExamine], "MessageID", jsondata.Id, AddSceneMessageCallBack);
...
function ChangeWorkerTime(time){
	dbBase.OpenTransaction(function(tx) {
		dbBase.SaveOrUpdateTable(tx,"tb_CurrentUsers",['LastTime'],[time],"UserID",userInfo.UserID);
	})
};
//网络连接验证时间
function ConfirmLoginTime(){
	dbBase.OpenTransaction(function(tx) {
		dbBase.SelectTable(tx,"select * from tb_CurrentUsers where Token !=?",[""],function(adata){
			if(adata){
				var _postdata={
					Token: adata.item(0).Token,
					LastTime: adata.item(0).LastTime
				}
				PostConfirmDays(_postdata,AlertOverDue);
			}
		})
	})
}
