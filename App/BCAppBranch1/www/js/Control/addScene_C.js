			...
			if (parentId == -1 || parentId == "-1" || parentId == null) {
				...
				function AddScene(adata) {
					if (adata.length >= 1) {
						...
						addScene_Control.SelectScene("ProjectID=? And ParentID=? And HasData=? And AllWorkers like '%" + userInfo.UserID + "%'", [_projectid, "-1", 'false'], _show.AddScene)
					}
				}
			} else {
				alltem = "";
				...
				function AddChild(adata) {
					...
					addScene_Control.SelectScene("ProjectID=? And ParentID=? And HasData=? And AllWorkers like '%" + userInfo.UserID + "%'", [projectId, parentId, 'false'], _show.AddScene);
				};
			}
		},
