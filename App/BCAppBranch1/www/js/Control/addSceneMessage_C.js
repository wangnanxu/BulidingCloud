...
function COLCallback(fileobj) {
	AddOneImg(fileobj.name, fileobj.URI);
}
...
function AddComplate(SignType, callback) {
	...
	dbBase.OpenTransaction(function(tx) {
		dbBase.SaveOrUpdateTable(tx, "tb_SceneMessageComments", ['MessageID', 'SceneID', 'UserID', 'UserPicture', 'UserPictureURI', 'UserName', 'Address', 'CreateTime', 'Description', 'Images', 'Comments', 'Status', 'Type', 'State', 'PictureGuid'], [_guid, _sceneid, userInfo.UserID, userInfo.HeadPictureURI, "", userInfo.UserName, null, _time, _description, _images, "", _status, _type, _state, ""], "MessageID", _guid, function(istrue) {
			if (istrue) {
