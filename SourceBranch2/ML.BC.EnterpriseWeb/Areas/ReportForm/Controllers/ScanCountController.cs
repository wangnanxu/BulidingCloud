...
using ML.BC.Web.Framework.Schedule;
...
{
    [AuthorizeCheck]
    public class ScanCountController : BCControllerBase
    {
        IScan service = null;
        public ScanCountController()
        {
            service = ML.BC.Infrastructure.Ioc.GetService<IScan>();
        }
        public ActionResult ScanIndex()
        {
            return View();
        }
        [PermissionControlAttribute(Functions.Root_ReportManagement_ScanCountDataReport_ShowAll)]
        public ActionResult GetList(string user)
        {
            var result = new StandardJsonResult<int []>();
            result.Try(() => {
                ScanCountModel list = service.GetScanCount(user);
                int[] ret = new int[4];
                if (list != null)
                {
                    ret[0] = list.ProjectNum;
                    ret[1] = list.CheckProjectNum;
                    ret[2] = list.SceneNum;
                    ret[3]=list.CheckSceneNum;
                    result.Value = ret;
                }
                else
        ...
        public List<FrontUserDto> GetUserList()
        {
            ...
            List<int?> departmentlist = new List<int?>();
                ...
                userlist = userservice.GetScanUsers(Functions.Root_ProjectManagement_SceneListManagement_Info,Functions.Root_ProjectManagement_ProjectListManagement_Info,BCSession.User.EnterpriseID,departmentlist);
