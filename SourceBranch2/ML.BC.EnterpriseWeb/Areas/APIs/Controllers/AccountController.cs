...
namespace ML.BC.EnterpriseWeb.Areas.APIs.Controllers
{
    public class AccountController : APIControllerBase
    {
        ...
        [AuthorizeCheck]
        [AllowCrossDomainPost]
        public ActionResult ChekLoginTime(LoginTimeCheckModel model)
        {
            var result = new StandardJsonResult<dynamic>();
            result.Try(() =>
            {
                ...
                if (flag)
                {
                    ...
                    appSyncService.ChangeUserMessageQueueStatus(BCSession.User.UserID, BCSession.User.Device, model.LastTime);
                }
                result.Value = new
                {
                    Overdue = flag
