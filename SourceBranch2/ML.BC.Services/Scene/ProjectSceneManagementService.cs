...
namespace ML.BC.Services
{
    ...
    {
        public List<ProjectDto> SearchProjectOnDepartment(string projectId, string projectName, string managerName, Status status, int? departmentID, string enterpriseId, int pageSize, int pageNumber, out int amount)
        {
            try
            {
        ...
        public bool UpdateProject(ProjectDto project)
        {
            try
            {
                using (var db = new BCEnterpriseContext())
                {
                    ...
                    temp.ProjectCode = project.ProjectCode;
        ...
        public ProjectAndSceneSyncDto GetProjectAndSceneForSync(string enterpriseId)
        {
            using (var db = new BCEnterpriseContext())
            {
                var query = (from ep in db.Enterprises
                             ...
                             join sp in db.SceneTypes on ep.EnterpriseID equals sp.EnterpriseID into tempSP
                             select new
                             {
                                 Projects = (
                                 from tpj in tempPJ
                ...
                var result = new ProjectAndSceneSyncDto
                {
                    ...
                    SceneTypes = query.SceneTypes.Select(o => new SceneTypeSyncDto()
                    {
                        SceneTypeID = o.ID,
                        SceneTypeName = o.Name,
                        ParentID = o.ParentID,
            ...
            try
            {
                using (var db = new BCEnterpriseContext())
                {
                    if (null == db.SyncStates.FirstOrDefault(o => (o.UserID == userID) && (o.DeviceID == deviceID)
                        && (o.ActionType == (byte)TypeEnum.Project || o.ActionType == (byte)TypeEnum.Scene || o.ActionType == (byte)TypeEnum.SceneType)))
                    {
                        ...
                        var syncSceneType = new SyncState() { ActionType = (byte)TypeEnum.SceneType, DeviceID = deviceID, UserID = userID };
                        AddSyncState(syncScene, syncProject, syncSceneType);
                    ...
                    var query = (from ep in db.Enterprises
                                 ...
                                 select new
                                 {
                                     ...
                                     SceneTypes = (
                                         from tsp in tempSP
                                         from st in db.SyncStates
                                         where st.UserID == userID && st.DeviceID == deviceID && st.ActionType == (byte)TypeEnum.SceneType && st.SyncTime < tsp.UpdateTime
                                         select new
                                         {
                                             tsp.ID,
                                             tsp.Name,
