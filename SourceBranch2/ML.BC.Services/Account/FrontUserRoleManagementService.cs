...
namespace ML.BC.Services
{
    public class FrontUserRoleManagementService : IFrontUserRoleManagementService
    {
        ...
        public List<KeyValuePair<string,List<string>>> GetUserRoleList(List<string> userList)
        {
            try
            {
                using (var db = new BCEnterpriseContext())
                {
                    var templist = db.UserRoles.Where(x => userList.Contains(x.UserID)).Select(n => new
                    {
                        uid = n.UserID,
                        rid = n.RoleID
                    }).ToList();
                    var list = from role in db.RFARoles
                        join user in templist on role.RoleID equals user.rid
                        select new
                        {
                            userid = user.uid,
                            rolename = role.Name
                        };
                    List<KeyValuePair<string,List<string>>> urlist = new List<KeyValuePair<string, List<string>>>();
                    foreach (var userole in list)
                    {
                        urlist.Add(new KeyValuePair<string, List<string>>(userole.userid, list.Where(x => x.userid.Equals(userole.userid)).Select(n => n.rolename).ToList()));
                    }
                    return urlist;
                }
            }
